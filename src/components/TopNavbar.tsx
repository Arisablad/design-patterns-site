            ...
            {expandMenu &&
                <div className={"bg-gray-800/20 shadow shadow-xl shadow-inner max-h-max md:hidden transition-all duration-[3000] ease-in-out pt-12 text-white"}>
                    {Links.map((section,index) => {
                        return (
                            <div key={index} className={"flex flex-col gap-2 px-1 py-1.5"}>
                                <p className={"text-2xl font-serif"}>{section.section_name}</p>
                                {section.links.map(link => <Link key={link.href} to={link.href} className={cn("flex w-full bg-gray-600 py-2 gap-2 px-1 hover:bg-blue-950/60 rounded-md", currentLocation === link.href && "bg-blue-900")}>
